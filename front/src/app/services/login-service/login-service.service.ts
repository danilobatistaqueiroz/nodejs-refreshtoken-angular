...
export class LoginServiceService {
  ...
  loginAuth(login:Login) {
    if (login.authcode) {
      this.headerOptions = new HttpHeaders({
        'x-tfa': login.authcode
      });
    }
    return this.http.post(`${environment.URL_API}/login`, { email: login.email, password: login.password }, { observe: 'response', headers: this.headerOptions, withCredentials: true });
  }

  registerUser(login:Login) {
    return this.http.post(`${environment.URL_API}/register`, { email: login.email, password: login.password, tfa: login.tfa }, { observe: "response" });
  }

  generateTfa(email:string) {
    return this.http.get(`${environment.URL_API}/tfa/generate?email=${email}`, { observe: 'response' });
  }
  ...
  verifyTfa(authcode: string) {
    return this.http.post(`${environment.URL_API}/tfa/verify`, { authcode }, { withCredentials: true, observe: 'response' });
  }
